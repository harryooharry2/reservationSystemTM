...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      if (error) {
        console.warn(
          'Enhanced availability function not available, using fallback:',
          error.message
        );
    ...
    if (party_size) {
      filteredData = data.filter(
        (table) => table.capacity >= parseInt(party_size)
      );
    }
...
router.get(
  '/',
  [verifyToken, logAuthEvent('RESERVATIONS_ACCESS')],
  asyncHandler(async (req, res) => {
...
router.get(
  '/:id',
  [verifyToken, logAuthEvent('RESERVATION_DETAIL_ACCESS')],
  asyncHandler(async (req, res) => {
    const { id } = req.params;
    const userRole = req.profile.role;
...
// POST /api/reservations - Create reservation with enhanced security validation
router.post(
  '/',
  [checkTokenBlacklist, ...reservationValidation, handleValidationErrors],
  async (req, res) => {
    try {
      ...
      if (!userId) {
        if (!customer_name || !customer_email) {
          return res.status(400).json({
            error:
              'Customer name and email are required for guest reservations',
          });
        }
        ...
        try {
          ...
          return res.status(500).json({
            error: 'Failed to process guest reservation',
          });
        }
      }

      // Create reservation using admin client with RLS bypass
      ...
      console.error('Unexpected error in reservation creation:', error);
      return res.status(500).json({
...
// PUT /api/reservations/:id - Update reservation with comprehensive validation
router.put(
  '/:id',
  [
    ...reservationValidation,
...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    const { data: reservationData, error: getErr } = await supabaseAdmin
      .from('reservations')
      .select(
        'id, user_id, table_id, reservation_date, start_time, end_time, status'
      )
      .eq('id', id)
      .single();
...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    socketManager.io
      .to(`user-reservations-${data.user_id}`)
      .emit('reservation-updated', updateData);
    socketManager.io
      .to('admin-dashboard')
      .emit('reservation-updated', updateData);
